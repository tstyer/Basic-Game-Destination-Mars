...
if (globalThis && globalThis.document && globalThis.jQuery) {
    const $ = globalThis.jQuery; // safe local alias
...
function getSupportBottomOnImage(bugW, supportEps) {
    const eps = (typeof supportEps === "number" ? supportEps : 8);
...
function updatePlatforms() {
    ...
    platforms = platforms.filter(function (platform) {
        const top = parseFloat(
            platform.style.top || String(platform.offsetTop)
        ) || 0;
